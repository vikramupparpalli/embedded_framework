...
static bool for_each_find_insertion_index(linked_list_node_t *node, void *_context, const size_t index)
{
...
static void add_timer(timer_module_t *module, timer_t *timer)
{
   ...
   linked_list_for_each(&module->timers, for_each_find_insertion_index, &context);

   linked_list_insert(&module->timers, &timer->node, context.index);
}

static void remove_timer(timer_module_t *module, timer_t *timer)
{
...
{
   remove_timer(module, timer);
}

static timer_ticks_t pending_ticks(timer_module_t *module)
{
...
static void start_timer(timer_module_t *module, timer_t *timer, const timer_ticks_t ticks, timer_callback_t callback, void *context)
{
   timer_ticks_t timerDuration = TRUNCATE_UNSIGNED_ADDITION(ticks, pending_ticks(module), MAX_TICKS);
...
void timer_module_start_oneshot(timer_module_t *module, timer_t *timer, const timer_ticks_t ticks, timer_callback_t callback, void *context)
{
   ...
   start_timer(module, timer, ticks, callback, context);
}
...
void timer_module_resume(timer_module_t *module, timer_t *timer)
{
   ...
   {
      timer_ticks_t remainingTicks = TRUNCATE_UNSIGNED_ADDITION(timer->ticks.pause, pending_ticks(module), MAX_TICKS);
      timer->paused = false;
...
timer_ticks_t timer_module_remaining_ticks(timer_module_t *module, timer_t *timer)
{
   if(timer_module_is_running(module, timer))
   {
      ...
      else
      {
         ...
         timer_ticks_t pending = pending_ticks(module);
...
timer_ticks_t timer_module_ticks_since_last_started(timer_module_t *module, timer_t *timer)
{
   ...
   return module->lastTicks - (timer->ticks.expiration - timer->startTicks) + pending_ticks(module);
}
...
static bool for_each_update_expired(linked_list_node_t *node, void *_context, const size_t index)
{
...
static void execute_timer(timer_module_t *module, timer_t *timer)
{
   if(!timer->autoReload)
   {
      timer_module_stop(module, timer);
...
static bool for_each_execute(linked_list_node_t *node, void *_context, const size_t index)
{
   REINTERPRET(timer, node, timer_t *);
   ...
   if(!timer->paused && timer->expired)
   {
      execute_timer(context->module, timer);
...
{
   {
      ...
      linked_list_for_each(&module->timers, for_each_update_expired, &context);
   }

   {
      ...
      linked_list_for_each(&module->timers, for_each_execute, &context);
